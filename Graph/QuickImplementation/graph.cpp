...
! Also remember that in DFS the time variable must be passed as reference because the time variable must be same for each
! DFS call
...
void graph:: bfs(int src){
    ...
    parent[src]=-1;
    dist_from_src[src]=0;
    visited[src]=1;//* Remember that in bfs the source is visited here. In Dijkstra's the source is processed after it is taken out
    //* of the queue
...
/* 
* We don't need to have the color logic in case of DFS. */
