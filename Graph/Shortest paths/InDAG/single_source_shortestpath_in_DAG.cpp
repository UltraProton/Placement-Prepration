...
class Graph{
    ...
    void topological_sort(vector<int> &topological_order);
    void topological_sort_helper(int src, vector<int> &Visited, vector<int> &topological_order);
...
void Graph :: topological_sort(vector<int> &topological_order){
    //* topological sort is same as dfs
    vector<int> Visited(V,0);
    for(int i=0;i<V;i++){
        if(!Visited[i]){
            topological_sort_helper(i,Visited,topological_order);
        }
    }
...
void Graph :: topological_sort_helper(int src, vector<int> &Visited, vector<int> &topological_order){
    Visited[src]=1;
    for(auto x:adj[src]){
        if(!Visited[x.first]){
            topological_sort_helper(x.first,Visited, topological_order);
        }
    }
